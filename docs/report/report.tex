...
BM25 only           & 0.6434 & 0.8023 & 0.8023 & 0.8023 & 0.7597 & 0.7660 & \textbf{0.74} \\
Hybrid (RRF)        & 0.6434 & 0.8023 & 0.8488 & \textbf{1.0000} & 0.7910 & 0.7862 & 15.91 \\
\textbf{Hybrid + Reranker} & \textbf{0.7946} & \textbf{0.8953} & 0.9070 & \textbf{1.0000} & \textbf{0.9085} & \textbf{0.8937} & 693.19 \\
\bottomrule
\end{tabular}
\end{center}
...
\toprule
\textbf{Recall@1} & \textbf{TF-IDF} & \textbf{Dense (MiniLM)} & \textbf{BM25} & \textbf{Hybrid + Rerank} & \textit{ceiling} \\
\midrule
\code{keyword}     & \textbf{1.0000} & 0.4286 & \textbf{1.0000} & \textbf{1.0000} & \textit{1.0000} \\
\code{paraphrased} & 0.1111 & \textbf{0.4444} & 0.1111 & \textbf{0.4444} & \textit{1.0000} \\
\code{terminology} & 0.8333 & 0.3333 & 0.8333 & \textbf{1.0000} & \textit{1.0000} \\
\code{factual}     & 0.8571 & 0.9286 & 0.8571 & \textbf{1.0000} & \textit{1.0000} \\
\code{multi\_step} & 0.4524 & 0.4524 & 0.3810 & \textbf{0.4524} & \textit{\textbf{0.4524}} \\
\midrule
\textbf{all answerable} & 0.6550 & 0.5853 & 0.6434 & \textbf{0.7946} & \textit{0.9109} \\
\bottomrule
\end{tabular}
\end{center}
...
\subsection{A metric read against the wrong ceiling}

Recall@$k$ is bounded by $\min(k, |R|) / |R|$, where $R$ is the set of relevant
chunks. A question with two relevant chunks cannot score above 0.5000 at $k=1$;
one with three cannot exceed 0.3333. Every \code{multi\_step} question in this
dataset has two or three, which puts the ceiling for that column at exactly
0.4524.

The reranked pipeline scores exactly 0.4524, with Recall@5 of 1.0000 and MRR of
1.0000.

\begin{finding}{The weakest-looking category was the most solved one}
\code{multi\_step} is not the weakest answerable category. It is saturated ---
every relevant chunk for every multi-step question is retrieved, with one of them
always at rank 1. This project's own documentation called it ``the weakest
answerable category'' and ``the honest next problem'' in four separate places,
because 0.4524 was read against an imagined 1.0 instead of against the bound the
labels impose. The evaluator now reports \code{max\_recall@k} beside
\code{recall@k}, so the number cannot be misread the same way again.
\end{finding}

Correcting the bound changes the overall reading as well. Across all 43
answerable questions the achievable Recall@1 is \textbf{0.9109}, not 1.0, so the
reranked pipeline's 0.7946 is \textbf{87.2\%} of what any retriever could reach
on this dataset. The genuinely open category is \code{paraphrased}, at 0.4444
against a ceiling of 1.0000 --- and unlike \code{multi\_step}, that gap is real.
...
\begin{itemize}[leftmargin=1.2em,itemsep=3pt]
  \item \textbf{Lint, types and tests} --- \code{ruff}, \code{mypy} over
        \code{app/}, \code{scripts/} and \code{frontend/}, and 170 tests behind a
...
\begin{enumerate}[leftmargin=1.4em,itemsep=3pt]
  ...
  \item \textbf{Reranking dominates latency} --- 693.19\,ms against BM25's
        0.74\,ms. Any deployment must decide whether that precision is worth
  ...
  \item \textbf{\code{paraphrased} is the weakest answerable category} (0.4444
        Recall@1 against a ceiling of 1.0000). Vocabulary mismatch between
        question and document is the one failure neither retriever handles well
        and reranking only partly repairs.
\end{enumerate}
...
\begin{enumerate}[leftmargin=1.4em,itemsep=3pt]
  ...
  \item \textbf{Attack \code{paraphrased}} --- now that the recall ceiling is
        computed, it is the only answerable category with real headroom.
...
\code{tests/}          & 170 unit and integration tests \\
